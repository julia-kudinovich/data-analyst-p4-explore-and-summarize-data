...
Form above we see that all the variebles have negative correlation with quality.
...
ggplot(aes(y=volatile.acidity, x=total.sulfur.dioxide),data=wine) +
  geom_jitter()+
  facet_wrap(~quality.factor)
```

It looks like better quality wines tend to concentrate in the lower left corner.
...
However, in the analysis we found several relationships between wine quality
...
Also, I did not find a relationship between pH level and qualty.That partuclar 
variable was among the ones I though would have a great impact on quality. 
